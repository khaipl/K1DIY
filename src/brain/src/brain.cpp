...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    player_id_desc.integer_range[0].to_value = MAX_NUM_PLAYERS;
...
void Brain::init()
{
    ...
    gameControlSubscription = create_subscription<game_controller_interface::msg::GameControlData>("/booster_soccer/game_controller" + topic_suffix, SUB_STATE_QUEUE_SIZE, bind(&Brain::gameControlCallback, this, _1));
...
void Brain::gameControlCallback(const game_controller_interface::msg::GameControlData &msg)
{
    ...
    bool isKickOffSide = (msg.kicking_team == teamId); // Whether our team is the kickoff side
    ...
    string gameSubState = "SET"; // TODO: Enlever substate du behavior tree?? Défini par défaut à cause que behavior tree a besoin d'un substate
    tree->setEntry<string>("gc_game_sub_state_type", gameSubStateType);
    tree->setEntry<string>("gc_game_sub_state", gameSubState);
    bool isSubStateKickOffSide = (msg.kicking_team == teamId); // For example, if the current secondary state is a free kick, whether our team is taking the free kick
