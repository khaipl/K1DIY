...
void BrainCommunication::unicastToGameController() {
    while (_unicast_gamecontrol_flag.load(std::memory_order_relaxed))
    {
        ...
        gc_return_data.playerNum = brain->config->get_player_id(); // return data id is 1,2,3,4
        
        // On suppose que s'il n'est pas "IS_READY", c'est qu'il est tombé ou en train de se relever.
        gc_return_data.fallen = (brain->data->recoveryState == RobotRecoveryState::IS_READY) ? 0 : 1;
        
        //TODO: ajouter informations sur balle et pose (selon RoboCupGameControlReturnData)
...
void BrainCommunication::spinCommunicationReceiver() {
    ...
    while (_receive_communication_flag.load(std::memory_order_relaxed)) {
        ...
        if (tmIdx < 0 || tmIdx >= MAX_NUM_PLAYERS) { // MAX_NUM_PLAYERS is the maximum number of players
